...
// var bou = [118.53, 29.32, 118.7, 29.67]; //下载范围
// 70, 22, 130, 60
// var bou = [70, 22, 95, 60]; //下载范围
// var bou = [95, 24, 130, 40]; //下载范围
var bou = [118.5639, 29.61714, 119.9963, 30.51254]; // 分水江下载范围
var Minlevel = 10; //最小层级
var Maxlevel = 10; //最大层级
// var token = '0b79a07d2808103ab84aa56485c331a8'; //天地图key(如果失效去天地图官网申请)
// var token = "b6be6cdcece979cac36070e2298e4a04";
let token = "5d82ea252b9bcd7c70a48697f2f14a5a";
  ...
  for (let z = 0; z <= all.length - 1; z++) {
    for (let x = all[z].x[0]; x <= all[z].x[1]; x++) {
      for (let y = all[z].y[0]; y <= all[z].y[1]; y++) {
        // 将下载任务推入队列
        let _pngName = `${zpath}/${all[z].t}/${x}/${y}.png`;
        if (fs.existsSync(_pngName)) {
          // console.log(`${_pngName}文件已经存在`);
...
function download(x, y, z) {
  ...
  // let imgurl =`http://t0.tianditu.gov.cn/DataServer?T=img_w&x=${x}&y=${y}&l=${z}&tk=${token}`
  var ip =
    Math.floor(Math.random() * 256) + //随机生成IP迷惑自己
    "." +
    ...
    "." +
    Math.floor(Math.random() * 256);
  var v = Math.floor(user_agent_list_2.length * Math.random());
  let _host = Math.floor(Math.random() * 1000);
  var options = {
    method: "GET",
    url: imgurl,
    headers: {
  ...
  let ws = fs
    .createWriteStream(`${zpath}/${z}/${x}/${y}.png`)
    .on("finish", () => {
